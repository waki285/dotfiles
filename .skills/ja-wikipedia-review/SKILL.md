...
- 記事内の出典だけで目安が確定しない場合、利益相反の開示がある場合、または出典が公式サイト・プレスリリースに偏る場合は、[notability.md](references/notability.md) の記事外調査を実行し、照会した窓口と件数、未確認の有料・館内データベース、主題に対して期待される資料と実際の出典との乖離を記録する。
...
改善案にも根拠を要する。特定の書式、表記、併記、テンプレート用法、リンク先の採用を勧める場合、その要求元の文書名と状態を示す。示せない案は要求として書かず、`慣行に基づく任意の提案` と明示し、現状のままでも許容されるかどうかを併せて書く。指摘本体に正しい根拠があることは、その指摘に付ける改善案の根拠を免除しない。
...
- 見慣れた書式や他記事の慣行を、現行の方針・ガイドラインに実在するか確認せずに要求しない。確認とは、対象の文書を開き、冒頭の状態表示を読み、該当節が実際に何を定めているかを引用できる形で特定することである。
- 文書中の用例は、その節が定める規則を説明するために置かれている。用例に現れる別の書式を、その節が定めていない一般規則として読み取らない。
- 提案中、草案、私論の文書にある規定を使う場合、文書状態と、その文書自身が付けている適用範囲の限定を両方示す。限定を落として一般の要求へ広げない。
